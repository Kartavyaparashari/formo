...
```bash
PHASE_INFO=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
