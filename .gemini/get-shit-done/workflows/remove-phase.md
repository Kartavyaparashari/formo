...
```bash
INIT=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "E:/PPCPLAN/FORMO/Formo/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
