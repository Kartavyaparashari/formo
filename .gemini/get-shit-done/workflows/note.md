...
- **Global scope**: `E:/PPCPLAN/FORMO/Formo/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/PPCPLAN/FORMO/Formo/.gemini/notes/`)
...
2. Glob `E:/PPCPLAN/FORMO/Formo/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (E:/PPCPLAN/FORMO/Formo/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/PPCPLAN/FORMO/Formo/.gemini/notes/` — works in any directory
